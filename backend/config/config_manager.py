...
class ConfigManager:
    ...
    def load_config(self) -> GlyphMindConfig:
        ...
        # Override with environment variables (for Render.com deployment)
        env_api_keys = {
        ...
        # Update API keys with environment variables (if they exist)
        for key, value in env_api_keys.items():
            if value:
                api_keys_data[key] = value
        
        # Server configuration from environment
        server_config = settings_data.get("server", {})
        server_config.update({
            "host": os.getenv("HOST", server_config.get("host", "127.0.0.1")),
        ...
        database_config.update({
            "knowledge_base_path": os.path.join(data_dir, "kb.sqlite"),
            "ledger_path": os.path.join(data_dir, "ledger.sqlite"),
