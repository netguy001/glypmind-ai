...
def make_api_request(endpoint: str, data: Dict[str, Any], method: str = "POST") -> Dict[str, Any]:
    ...
    try:
        ...
        return {"error": "Request timed out. The backend might be starting up, please try again."}
...
def get_system_status() -> str:
        ...
        if isinstance(ai_info, dict):
            for model_name, model_info in ai_info.items():
                if isinstance(model_info, dict):
                    status_icon = "✅" if model_info.get('health_check', False) else "❌"
        ...
        if isinstance(web_info, dict):
            for source_name, source_info in web_info.items():
                if isinstance(source_info, dict):
                    status_icon = "✅" if source_info.get('health_check', False) else "❌"
    ...
    if 'knowledge_base' in system_info:
        ...
        if isinstance(kb_info, dict):
            ...
            output += f"- **Recent Entries (7d):** {kb_info.get('recent_entries_7d', 0)}\n"
        output += "\n"
